...
#    ax.plot(1./T_array,-20*(np.tanh(1./T_array)),'-ko',lw=2,label='Large N 1D Ising Theory')
#    ax.plot(1./T_array,isingE(1./T_array,N=10),'-ko',lw=2,label='Exact 1D Ising Theory')    
...
def isingE(beta, N=20, epsilon=1):
    num=np.cosh(epsilon*beta)**(N-1) * np.sinh(epsilon*beta)+np.sinh(epsilon*beta)**(N-1) * np.cosh(epsilon*beta)
    den=np.cosh(epsilon*beta)**N + np.sinh(epsilon*beta)**N
    return -N*num/den
    
def theoryE(beta,epsilon=1,m=2,N=6):
