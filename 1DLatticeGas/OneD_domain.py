    ...
    def energy(self,state):
        ...
        for pos in xrange(self.params['m']-1):
            ...
            if state_sorted[0][pos+1]-state_sorted[0][pos]==1:
                E+=self.params['H_array'][state_sorted[1][pos][1]][state_sorted[1][pos+1][0]]
            else:
                # now since there is no protein at the next position,
                # add the two end energy 
                # end energy of the right-end domain of protein p
                E+=self.params['H_array'][state_sorted[1][pos][1]][self.params['k']]
                # end energy of the left-end domain of protein p+1
                E+=self.params['H_array'][state_sorted[1][pos+1][0]][self.params['k']]
    ...
    def change(self):
        ...
        if np.random.rand()<=ratio:
            ...
            if self.params['isAlt']:
                ...
                # and detect whether protein is at ends of space
                if self.state[0][position_idx]==self.params['N']-1:
                    rightPos=0
                    leftPos = self.state[0][position_idx]-1
                elif  self.state[0][position_idx]==0:
                    rightPos= self.state[0][position_idx]+1
                    leftPos = self.params['N']-1
                else: 
                    rightPos= self.state[0][position_idx]+1
                    leftPos = self.state[0][position_idx]-1
                #Perform change of position
                if u<= self.params['altProb'] and leftPos in self.state[0]:
                    changedPosition[position_idx]=changedPosition[position_idx]-1
                elif u> self.params['altProb'] and rightPos in self.state[0]:
                    changedPosition[position_idx]=changedPosition[position_idx]+1
            else:
                #where jumping through proteins is allowed.
                emptySlots = np.array([x for x in xrange(self.params['N']) if x not in self.state[0]])
                if len(emptySlots)!=0:
...
if __name__ == "__main__":
    ...
    #write to animation
#    TE_animate('1dGas_1',initParams,50,100)

                
    #Save result to file
    T_array=np.logspace(-3,3,num=20)
    TE_simulation('1DGasTestN400m300.p',initParams,T_array,simPerPt=1,obsStart=1000,obsDur=1000)